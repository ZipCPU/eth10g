...
// }}}
	...
	// Verilator lint_on  UNUSED
	// }}}
	// vpktmem stream definition(s)
	// {{{
	wire	vpktmem_axin_valid, vpktmem_axin_ready,
		vpktmem_axin_last, vpktmem_axin_abort;
	wire	[31:0]	vpktmem_axin_data;
	wire	[1:0]	vpktmem_axin_bytes;
	// }}}
	wire	[8-1:0]	w_led;
	// Verilator lint_off	UNUSED
	wire	[31:0]	cfg_debug;
...
// `include "builddate.v"
	...
	// vpktsrc stream definition(s)
	// {{{
	wire	vpktsrc_axin_valid, vpktsrc_axin_ready,
		vpktsrc_axin_last, vpktsrc_axin_abort;
	wire	[31:0]	vpktsrc_axin_data;
	wire	[1:0]	vpktsrc_axin_bytes;
	// }}}
...
// }}}
	...
	// Bus wbwide
	// {{{
	// Wishbone definitions for bus wbwide, component vpktmem
	// Verilator lint_off UNUSED
	wire		wbwide_vpktmem_cyc, wbwide_vpktmem_stb, wbwide_vpktmem_we;
	wire	[19:0]	wbwide_vpktmem_addr;
	wire	[511:0]	wbwide_vpktmem_data;
	wire	[63:0]	wbwide_vpktmem_sel;
	wire		wbwide_vpktmem_stall, wbwide_vpktmem_ack, wbwide_vpktmem_err;
	wire	[511:0]	wbwide_vpktmem_idata;
	// Verilator lint_on UNUSED
	...
	// Verilator lint_on UNUSED
	// Wishbone definitions for bus wbwide, component vpktsnk
	// Verilator lint_off UNUSED
	wire		wbwide_vpktsnk_cyc, wbwide_vpktsnk_stb, wbwide_vpktsnk_we;
	wire	[19:0]	wbwide_vpktsnk_addr;
	wire	[511:0]	wbwide_vpktsnk_data;
	wire	[63:0]	wbwide_vpktsnk_sel;
	wire		wbwide_vpktsnk_stall, wbwide_vpktsnk_ack, wbwide_vpktsnk_err;
	wire	[511:0]	wbwide_vpktsnk_idata;
	// Verilator lint_on UNUSED
	...
	// Verilator lint_on UNUSED
	// Wishbone definitions for bus wbwide, component vpktsrc
	// Verilator lint_off UNUSED
	wire		wbwide_vpktsrc_cyc, wbwide_vpktsrc_stb, wbwide_vpktsrc_we;
	wire	[19:0]	wbwide_vpktsrc_addr;
	wire	[511:0]	wbwide_vpktsrc_data;
	wire	[63:0]	wbwide_vpktsrc_sel;
	wire		wbwide_vpktsrc_stall, wbwide_vpktsrc_ack, wbwide_vpktsrc_err;
	wire	[511:0]	wbwide_vpktsrc_idata;
	// Verilator lint_on UNUSED
	...
	// Verilator lint_on UNUSED
	// Wishbone definitions for bus wb32(DIO), component vpktmem
	// Verilator lint_off UNUSED
	wire		wb32_vpktmem_cyc, wb32_vpktmem_stb, wb32_vpktmem_we;
	wire	[6:0]	wb32_vpktmem_addr;
	wire	[31:0]	wb32_vpktmem_data;
	wire	[3:0]	wb32_vpktmem_sel;
	wire		wb32_vpktmem_stall, wb32_vpktmem_ack, wb32_vpktmem_err;
	wire	[31:0]	wb32_vpktmem_idata;
	// Verilator lint_on UNUSED
	// Wishbone definitions for bus wb32(DIO), component vpktsnk
	// Verilator lint_off UNUSED
	wire		wb32_vpktsnk_cyc, wb32_vpktsnk_stb, wb32_vpktsnk_we;
	wire	[6:0]	wb32_vpktsnk_addr;
	wire	[31:0]	wb32_vpktsnk_data;
	wire	[3:0]	wb32_vpktsnk_sel;
	wire		wb32_vpktsnk_stall, wb32_vpktsnk_ack, wb32_vpktsnk_err;
	wire	[31:0]	wb32_vpktsnk_idata;
	// Verilator lint_on UNUSED
	// Wishbone definitions for bus wb32(DIO), component vpktsrc
	// Verilator lint_off UNUSED
	wire		wb32_vpktsrc_cyc, wb32_vpktsrc_stb, wb32_vpktsrc_we;
	wire	[6:0]	wb32_vpktsrc_addr;
	wire	[31:0]	wb32_vpktsrc_data;
	wire	[3:0]	wb32_vpktsrc_sel;
	wire		wb32_vpktsrc_stall, wb32_vpktsrc_ack, wb32_vpktsrc_err;
	wire	[31:0]	wb32_vpktsrc_idata;
	// Verilator lint_on UNUSED
	...
	wbwide_xbar(
		...
		.o_mstall({
			wbwide_i2cm_stall,
			wbwide_vpktsrc_stall,
			wbwide_zip_stall,
			wbwide_vpktsnk_stall,
			wbwide_wbu_arbiter_stall,
			wbwide_vpktmem_stall
		}),
		.o_mack({
			wbwide_i2cm_ack,
			wbwide_vpktsrc_ack,
		...
		.o_mdata({
			wbwide_i2cm_idata,
			wbwide_vpktsrc_idata,
			wbwide_zip_idata,
			wbwide_vpktsnk_idata,
			wbwide_wbu_arbiter_idata,
			wbwide_vpktmem_idata
		}),
		.o_merr({
			wbwide_i2cm_err,
			wbwide_vpktsrc_err,
	...
	3'b10?: r_wb32_dio_bus_select <= 3'd4;
	...
	3'd1: r_wb32_dio_data <= wb32_vpktmem_idata;
	3'd2: r_wb32_dio_data <= wb32_vpktsnk_idata;
	3'd3: r_wb32_dio_data <= wb32_vpktsrc_idata;
	...
	assign	wb32_i2cs_stb = wb32_dio_stb && ((wb32_dio_addr[ 4: 2] &  3'h7) ==  3'h0);  // 0x00 - 0x0f
	...
	assign	wb32_vpktmem_cyc = wb32_dio_cyc;
	assign	wb32_vpktmem_stb = wb32_dio_stb && ((wb32_dio_addr[ 4: 2] &  3'h7) ==  3'h1);  // 0x10 - 0x1f
	assign	wb32_vpktmem_we  = wb32_dio_we;
	assign	wb32_vpktmem_addr= wb32_dio_addr;
	assign	wb32_vpktmem_data= wb32_dio_data;
	assign	wb32_vpktmem_sel = wb32_dio_sel;
	assign	wb32_vpktsnk_cyc = wb32_dio_cyc;
	assign	wb32_vpktsnk_stb = wb32_dio_stb && ((wb32_dio_addr[ 4: 2] &  3'h7) ==  3'h2);  // 0x20 - 0x2f
	assign	wb32_vpktsnk_we  = wb32_dio_we;
	assign	wb32_vpktsnk_addr= wb32_dio_addr;
	assign	wb32_vpktsnk_data= wb32_dio_data;
	assign	wb32_vpktsnk_sel = wb32_dio_sel;
	assign	wb32_vpktsrc_cyc = wb32_dio_cyc;
	assign	wb32_vpktsrc_stb = wb32_dio_stb && ((wb32_dio_addr[ 4: 2] &  3'h7) ==  3'h3);  // 0x30 - 0x3f
	assign	wb32_vpktsrc_we  = wb32_dio_we;
	assign	wb32_vpktsrc_addr= wb32_dio_addr;
	assign	wb32_vpktsrc_data= wb32_dio_data;
	assign	wb32_vpktsrc_sel = wb32_dio_sel;
	assign	wb32_sio_cyc = wb32_dio_cyc;
	assign	wb32_sio_stb = wb32_dio_stb && ((wb32_dio_addr[ 4: 2] &  3'h6) ==  3'h4);  // 0x40 - 0x5f
	...
	//
	wbxbar #(
		.NM(1), .NS(10), .AW(7), .DW(32),
		.SLAVE_ADDR({
			// Address width    = 7
			// Address LSBs     = 2
			// Slave name width = 9
			{ 7'h60 }, //  wb32_dio: 0x180
			{ 7'h40 }, //       cfg: 0x100
			{ 7'h38 }, //       fan: 0x0e0
			{ 7'h30 }, //      uart: 0x0c0
			{ 7'h28 }, //    sdcard: 0x0a0
			{ 7'h20 }, // scope_smi: 0x080
			{ 7'h18 }, //  flashdbg: 0x060
			{ 7'h10 }, //  fanscope: 0x040
			{ 7'h08 }, //  cfgscope: 0x020
			{ 7'h00 }  //  flashcfg: 0x000
		}),
		.SLAVE_MASK({
			// Address width    = 7
			// Address LSBs     = 2
...
`endif	// FAN_ACCESS
	...
	pktvfifo #(
		...
		.i_clk(i_clk),
		.i_reset(i_reset),
		.i_ctrl_cyc(wb32_vpktmem_cyc), .i_ctrl_stb(wb32_vpktmem_stb), .i_ctrl_we(wb32_vpktmem_we),
			.i_ctrl_addr(wb32_vpktmem_addr[2-1:0]),
			.i_ctrl_data(wb32_vpktmem_data), // 32 bits wide
			.i_ctrl_sel(wb32_vpktmem_sel),  // 32/8 bits wide
		.o_ctrl_stall(wb32_vpktmem_stall),.o_ctrl_ack(wb32_vpktmem_ack), .o_ctrl_data(wb32_vpktmem_idata),
		.S_VALID(vpktsrc_axin_valid),
		.S_READY(vpktsrc_axin_ready),
		.S_DATA(vpktsrc_axin_data),
		.S_BYTES(vpktsrc_axin_bytes),
		.S_LAST(vpktsrc_axin_last),
		.S_ABORT(vpktsrc_axin_abort),
		.o_wb_cyc(wbwide_vpktmem_cyc), .o_wb_stb(wbwide_vpktmem_stb), .o_wb_we(wbwide_vpktmem_we),
			.o_wb_addr(wbwide_vpktmem_addr[20-1:0]),
			.o_wb_data(wbwide_vpktmem_data), // 512 bits wide
			.o_wb_sel(wbwide_vpktmem_sel),  // 512/8 bits wide
		.i_wb_stall(wbwide_vpktmem_stall), .i_wb_ack(wbwide_vpktmem_ack), .i_wb_data(wbwide_vpktmem_idata), .i_wb_err(wbwide_vpktmem_err),
		.M_VALID(vpktmem_axin_valid),
		.M_READY(vpktmem_axin_ready),
		.M_DATA(vpktmem_axin_data),
		.M_BYTES(vpktmem_axin_bytes),
		.M_LAST(vpktmem_axin_last),
		.M_ABORT(vpktmem_axin_abort)
		// }}}
	);
	////////////////////////////////////////////////////////////////////////
	//
...
`endif	// FLASHCFG_ACCESS
	...
	pkt2mem #(
		...
		.i_clk(i_clk),
		.i_reset(i_reset),
		.i_wb_cyc(wb32_vpktsnk_cyc), .i_wb_stb(wb32_vpktsnk_stb), .i_wb_we(wb32_vpktsnk_we),
			.i_wb_addr(wb32_vpktsnk_addr[2-1:0]),
			.i_wb_data(wb32_vpktsnk_data), // 32 bits wide
			.i_wb_sel(wb32_vpktsnk_sel),  // 32/8 bits wide
		.o_wb_stall(wb32_vpktsnk_stall),.o_wb_ack(wb32_vpktsnk_ack), .o_wb_data(wb32_vpktsnk_idata),
		.S_AXIN_VALID(vpktmem_axin_valid),
		.S_AXIN_READY(vpktmem_axin_ready),
		.S_AXIN_DATA( vpktmem_axin_data),
		.S_AXIN_BYTES(vpktmem_axin_bytes),
		.S_AXIN_LAST( vpktmem_axin_last),
		.S_AXIN_ABORT(vpktmem_axin_abort),
		.o_dma_cyc(wbwide_vpktsnk_cyc), .o_dma_stb(wbwide_vpktsnk_stb), .o_dma_we(wbwide_vpktsnk_we),
			.o_dma_addr(wbwide_vpktsnk_addr[20-1:0]),
			.o_dma_data(wbwide_vpktsnk_data), // 512 bits wide
			.o_dma_sel(wbwide_vpktsnk_sel),  // 512/8 bits wide
		.i_dma_stall(wbwide_vpktsnk_stall), .i_dma_ack(wbwide_vpktsnk_ack), .i_dma_data(wbwide_vpktsnk_idata), .i_dma_err(wbwide_vpktsnk_err),
		.o_int(vpktsnk_int)
		// }}}
	);
	// }}}
...
`endif	// SDSPI_ACCESS
	...
	mem2pkt #(
		...
		.i_clk(i_clk),
		.i_reset(i_reset),
		.i_wb_cyc(wb32_vpktsrc_cyc), .i_wb_stb(wb32_vpktsrc_stb), .i_wb_we(wb32_vpktsrc_we),
			.i_wb_addr(wb32_vpktsrc_addr[2-1:0]),
			.i_wb_data(wb32_vpktsrc_data), // 32 bits wide
			.i_wb_sel(wb32_vpktsrc_sel),  // 32/8 bits wide
		.o_wb_stall(wb32_vpktsrc_stall),.o_wb_ack(wb32_vpktsrc_ack), .o_wb_data(wb32_vpktsrc_idata),
		.o_dma_cyc(wbwide_vpktsrc_cyc), .o_dma_stb(wbwide_vpktsrc_stb), .o_dma_we(wbwide_vpktsrc_we),
			.o_dma_addr(wbwide_vpktsrc_addr[20-1:0]),
			.o_dma_data(wbwide_vpktsrc_data), // 512 bits wide
			.o_dma_sel(wbwide_vpktsrc_sel),  // 512/8 bits wide
		.i_dma_stall(wbwide_vpktsrc_stall), .i_dma_ack(wbwide_vpktsrc_ack), .i_dma_data(wbwide_vpktsrc_idata), .i_dma_err(wbwide_vpktsrc_err),
		.M_AXIN_VALID(vpktsrc_axin_valid),
		.M_AXIN_READY(vpktsrc_axin_ready),
		.M_AXIN_DATA(vpktsrc_axin_data),
		.M_AXIN_BYTES(vpktsrc_axin_bytes),
		.M_AXIN_LAST(vpktsrc_axin_last),
		.M_AXIN_ABORT(vpktsrc_axin_abort),
		.o_int(vpktsrc_int)
		// }}}
	);
	// }}}
