...
`endif
	...
	wire		i_emmc_cd_n;
	wire		o_emmc_clk;
	wire	[3:0]	su_nc;	// Unused / noconnect wires
`ifdef	VERILATOR
	genvar		g_emmc;
	wire		w_emmc_cmd_tristate;
	wire		i_emmc_cmd, o_emmc_cmd;
	wire	[8-1:0]	w_emmc_dat_tristate;
	wire	[8-1:0]	i_emmc_dat, o_emmc_dat;
`endif
	// }}}
	// I2CCPU definitions
	// {{{
...
`ifdef	VERILATOR
	...
	wire		i_sdio_cmd, o_sdio_cmd;
	wire	[4-1:0]	w_sdio_dat_tristate;
	wire	[4-1:0]	i_sdio_dat, o_sdio_dat;
`endif
	// }}}
...
`ifdef	VERILATOR
	assign	io_emmc_cmd = (w_emmc_cmd_tristate) ? 1'bz
				: o_emmc_cmd;
	assign	i_emmc_cmd = io_emmc_cmd;

	generate for(g_emmc=0; g_emmc<8; g_emmc=g_emmc+1)
	begin : GEN_emmc_IO
		assign	io_emmc_dat[g_emmc] = (w_emmc_dat_tristate[g_emmc]) ? 1'bz : o_emmc_dat[g_emmc];
	end endgenerate

	assign	i_emmc_dat = io_emmc_dat;
`endif
	// }}}
...
`ifdef	VERILATOR
	assign	io_sdcard_cmd = (w_sdio_cmd_tristate) ? 1'bz
				: o_sdio_cmd;
	assign	i_sdio_cmd = io_sdcard_cmd;

	generate for(g_sdio=0; g_sdio<4; g_sdio=g_sdio+1)
	begin : GEN_sdio_IO
		assign	io_sdcard_dat[g_sdio] = (w_sdio_dat_tristate[g_sdio]) ? 1'bz : o_sdio_dat[g_sdio];
	end endgenerate

	assign	i_sdio_dat = io_sdcard_dat;
`endif
