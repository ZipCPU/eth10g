...
`endif
	...
	output	wire	o_cruvmmc_clk;

	input	wire		i_cruvmmc_ds;


	inout	wire		io_cruvmmc_cmd;
	inout	wire	[8-1:0]	io_cruvmmc_dat;
	// }}}
	...
	// {{{
	wire		w_cruvmmc_hwreset_n;
	wire		w_cruvmmc_cfg_ddr;
	wire		w_cruvmmc_cfg_ds, w_cruvmmc_cfg_dscmd;
	wire	[4:0]	w_cruvmmc_cfg_sample_shift;
	wire		w_cruvmmc_pp_cmd;
	wire		w_cruvmmc_pp_data;
		//
	wire	[7:0]	w_cruvmmc_sdclk;
	wire		w_cruvmmc_cmd_en;
	wire	[1:0]	w_cruvmmc_cmd_data;
	wire		w_cruvmmc_data_en;
	wire		w_cruvmmc_rx_en;
	wire	[31:0]	w_cruvmmc_tx_data;
		//
	wire	[1:0]	w_cruvmmc_cmd_strb;
	wire	[1:0]	w_cruvmmc_cmd_idata;
	wire		w_cruvmmc_cmd_collision;
	wire		w_cruvmmc_card_busy;
	wire	[1:0]	w_cruvmmc_rx_strb;
	wire	[15:0]	w_cruvmmc_rx_data;
		//
	wire		w_cruvmmc_ac_valid;
	wire	[1:0]	w_cruvmmc_ac_data;
	wire		w_cruvmmc_ad_valid;
	wire	[31:0]	w_cruvmmc_ad_data;

	wire		w_cruvmmc_ck;
	wire		w_cruvmmc_ds;
	wire	[31:0]	w_cruvmmc_debug;
	// }}}

	wire	[3:0]	su_cruvmmc_nc;	// Unused / noconnect wires
	wire	i_cruvmmc_cd_n;

	assign	i_cruvmmc_cd_n = 1'b0;
...
`endif
	...
	sdfrontend #(
		...
		// Configuration
		.i_cfg_ddr(w_cruvmmc_cfg_ddr),
		.i_cfg_ds(w_cruvmmc_cfg_ds),
		.i_cfg_dscmd(w_cruvmmc_cfg_dscmd),
		.i_sample_shift(w_cruvmmc_cfg_sample_shift),
		.i_pp_cmd(w_cruvmmc_pp_cmd),
		.i_pp_data(w_cruvmmc_pp_data),
		// Run-time inputs
		.i_sdclk(w_cruvmmc_sdclk),
		.i_cmd_en(w_cruvmmc_cmd_en),
		.i_cmd_data(w_cruvmmc_cmd_data),
		.i_data_en(w_cruvmmc_data_en),
		.i_rx_en(w_cruvmmc_rx_en),
		.i_tx_data(w_cruvmmc_tx_data),
		// Return values
		.o_cmd_strb(w_cruvmmc_cmd_strb),
		.o_cmd_data(w_cruvmmc_cmd_idata),
		.o_cmd_collision(w_cruvmmc_cmd_collision),
		.o_data_busy(w_cruvmmc_card_busy),
		.o_rx_strb( w_cruvmmc_rx_strb),
		.o_rx_data( w_cruvmmc_rx_data),
		//
		.MAC_VALID(w_cruvmmc_ac_valid),
		.MAC_DATA( w_cruvmmc_ac_data),
		.MAD_VALID(w_cruvmmc_ad_valid),
		.MAD_DATA( w_cruvmmc_ad_data),
		// IO ports
		.o_ck(w_cruvmmc_ck),
		.i_ds(w_cruvmmc_ds),
		.io_cmd(io_cruvmmc_cmd),
		.io_dat(io_cruvmmc_dat),
		.o_debug(w_cruvmmc_debug)
		// }}}
	);

	assign	o_cruvmmc_hwreset_n = w_cruvmmc_hwreset_n;

	assign	o_cruvmmc_clk = w_cruvmmc_ck;

	assign	w_cruvmmc_ds = i_cruvmmc_ds;
