...
public:
	...
	MAINTB(void) {
		...
		// From cruvmmc
#ifdef	EMMC_ACCESS
		m_cruvmmc = new SDIOSIM("emmc.img");
#endif
		// From emmc
#ifdef	EMMC_ACCESS
	...
	virtual	void	sim_clk_tick(void) {
		...
		//
		// SIM.TICK from cruvmmc
#ifdef	EMMC_ACCESS
		{ unsigned	tmp, tmp_async;
		m_cruvmmc->apply(
			(unsigned)m_core->o_cruvmmc_sdclk,
			(unsigned)m_core->o_cruvmmc_cfg_ddr,
			(unsigned)m_core->o_cruvmmc_cmd_en,
			(unsigned)m_core->o_cruvmmc_cmd_data,
			(unsigned)m_core->o_cruvmmc_data_en,
			(unsigned)m_core->o_cruvmmc_rx_en,
			(unsigned)m_core->o_cruvmmc_tx_data,
			tmp, tmp_async,
			m_core->i_cruvmmc_ad_data);
		m_core->i_cruvmmc_cmd_strb = (tmp >> 30) & 3;
		m_core->i_cruvmmc_cmd_data = (tmp >> 28) & 3;
		m_core->i_cruvmmc_rx_strb  = (tmp >> 24) & 3;
		m_core->i_cruvmmc_rx_data  =  tmp & 0x0ffff;
		m_core->i_cruvmmc_ac_valid = (tmp_async & 2) ? 1:0;
		m_core->i_cruvmmc_ad_valid =  tmp_async & 1;
		if (!m_core->o_cruvmmc_cfg_ds) {
			m_core->i_cruvmmc_ad_valid = 0;
			m_core->i_cruvmmc_ad_data = 0;
		} }
#endif
		// SIM.TICK from emmc
#ifdef	EMMC_ACCESS
